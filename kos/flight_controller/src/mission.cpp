...
void printMission()
{
    ...
    for (int i = 0; i < commandNum; i++)
    {
        ...
        case CommandType::SET_SERVO:
            fprintf(stderr, "[%s] Info: Set servo: %d, %d, coords: %d %d %d\n", ENTITY_NAME, commands[i].content.servo.number,
                    commands[i].content.servo.pwm, commands[i].content.waypoint.latitude,
                    commands[i].content.waypoint.longitude, commands[i].content.waypoint.altitude);
            break;
        default:
...
{
    uint32_t cNum = commandNum;
    for (uint32_t i = waypoint; i < commandNum; i++)
    {
        switch (type)
        {
        case 'H':
